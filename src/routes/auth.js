...
router.post('/register', async (req, res, next) => {
    try {
        ...
        try {
            await sendWelcomeEmail(user.email, user.firstName, verificationToken, req.app);
        } catch (emailError) {
...
router.post('/forgot', async (req, res, next) => {
    try {
        ...
        // Send password reset email
        try {
            await sendPasswordResetEmail(user.email, user.firstName, resetToken, req.app);
        } catch (emailError) {
...
router.post('/magic-link', async (req, res, next) => {
    try {
        ...
        try {
            await sendMagicLinkEmail(user.email, user.firstName, magicToken, isNewUser, req.app);
        } catch (emailError) {
